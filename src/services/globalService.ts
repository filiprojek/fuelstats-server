...
export class Err implements ErrType {
  ...
  drop() {
    if (this.data !== null && this.data !== undefined) {
      ...
      console.log(
        colors.bgRed(`${this.code}`) +
          colors.bgBlack.red(` ${this.message}`) +
          ' ' +
          colors.red(dataStr)
      );
    ...
    console.log(
      colors.bgRed(`${this.code}`) + colors.bgBlack.red(` ${this.message}`)
    );
...
export class Succ {
  ...
  drop() {
    if (this.data !== null && this.data !== undefined) {
      ...
      console.log(
        colors.bgGreen.black(`${this.code}`) +
          colors.green.bgBlack(` ${this.message}`) +
          ' ' +
          colors.green(dataStr)
      );
      return {
        code: this.code,
        message: this.message,
...
export class Log implements LogType {
  ...
  static dateNow(): string {
    ...
    return `${d.getFullYear()}-${add0(d.getMonth() + 1)}-${add0(
      d.getDate()
    )} ${add0(d.getHours())}:${add0(d.getMinutes())}:${add0(
      d.getSeconds()
    )}`;
  }
