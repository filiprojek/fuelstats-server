...
describe("UsersController – GET /api/v1/user/me", () => {
	...
	it("should return 401 if the token is invalid", async () => {
		const res = await request(app).get("/api/v1/user/me").set("Authorization", "Bearer invalid.token.here");
		expect(res.status).toBe(401);
	...
	it("should return the user profile for a valid token", async () => {
		...
		const token = jwt.sign({ sub: userId, email: userRecord.email }, env.JWT_SECRET, { expiresIn: "1h" });
